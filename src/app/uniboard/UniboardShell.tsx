...
export default function UniboardShell({
  ...
  useEffect(() => {
    const handleOpenUnibot = (e: Event) => {
      const customEvent = e as CustomEvent<UnibotImproveRequestDetail>;
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0a0a0a] font-sans text-slate-900 dark:text-slate-100 text-[13px] transition-colors duration-300">
              ...
              {NAV_ITEMS.map(({ href, label }) => {
                ...
                return (
                  <Link
                    key={href}
                    href={href}
                    className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all duration-200 whitespace-nowrap ${isActive
      ...
      <DebugConsole
        ...
        onAddPopup={() => alert(`Add new popup for ${pathname}`)}
      />
