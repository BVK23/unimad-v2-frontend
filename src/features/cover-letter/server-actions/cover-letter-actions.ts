"use server";

import { authedFetch } from "@/lib/authed-fetch";
import type { CoverLetterAsset, GenerateCoverLetterParams, UpdateCoverLetterParams } from "../types";

const ASSET_TYPE = "coverletter";

const normalizeCoverLetterAsset = (asset: CoverLetterAsset): CoverLetterAsset => ({
  ...asset,
  job_description: (asset.job_description ?? asset.jd ?? "").trim() || undefined,
});

// ---------------------------------------------------------------------------
// Server Actions
// ---------------------------------------------------------------------------

/**
 * Fetch all cover letters for the current user.
 * GET /api/application-assets/?type=coverletter
 */
export async function fetchCoverLetters(): Promise<CoverLetterAsset[]> {
  const res = await authedFetch(`/api/application-assets/?type=${ASSET_TYPE}`, { method: "GET" });
  if (res.status === 404) {
    return [];
  }
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error((data as { error?: string })?.error ?? "Failed to fetch cover letters");
  }
  const data = (await res.json()) as { assetData?: CoverLetterAsset[] };
  return (data.assetData ?? []).map(normalizeCoverLetterAsset);
}

/**
 * Fetch a single cover letter by id (tries detail endpoint, then list fallback).
 */
export async function fetchCoverLetterById(id: string | number): Promise<CoverLetterAsset | null> {
  try {
    const res = await authedFetch(`/api/application-assets/${ASSET_TYPE}/${id}/`, { method: "GET" });
    if (res.ok) {
      const detail = (await res.json()) as CoverLetterAsset;
      if (detail?.content != null || detail?.id != null) {
        return normalizeCoverLetterAsset(detail);
      }
    }
  } catch {
    // fall through to list
  }
  const list = await fetchCoverLetters();
  const asset = list.find(a => String(a.id) === String(id));
  return asset ?? null;
}

/**
 * Fetch cover letter and poll until content is available (e.g. after async generate).
 */
export async function fetchCoverLetterWithContent(
  id: string | number,
  maxRetries = 10,
  retryDelayMs = 1000
): Promise<CoverLetterAsset | null> {
  let asset = await fetchCoverLetterById(id);
  let retries = 0;
  while (retries < maxRetries && asset && (!asset.content || String(asset.content).trim().length === 0)) {
    await new Promise(r => setTimeout(r, retryDelayMs));
    asset = await fetchCoverLetterById(id);
    retries++;
  }
  return asset;
}

/**
 * Generate a new cover letter.
 * POST /api/application-assets/generate/
 * Returns { id } on success; throws or returns error shape for 409 / subscription.
 */
export async function generateCoverLetter(
  params: GenerateCoverLetterParams
): Promise<
  | { id: string | number }
  | { error: { data: { existing_asset_id: string | number }; message?: string } }
  | { error_code: "NOT_A_PLUS_MEMBER" }
> {
  const res = await authedFetch("/api/application-assets/generate/", {
    method: "POST",
    body: JSON.stringify({
      type: ASSET_TYPE,
      role: params.role,
      company: params.company,
      job_description: params.job_description,
      ...(params.application_id && { application_id: params.application_id }),
    }),
  });
  const data = await res.json().catch(() => ({}));
  if (res.status === 409) {
    return {
      error: {
        data: { existing_asset_id: (data as { existing_asset_id?: string })?.existing_asset_id ?? "" },
        message: (data as { message?: string })?.message,
      },
    };
  }
  if (res.status === 403 || (data as { error_code?: string })?.error_code === "NOT_A_PLUS_MEMBER") {
    return { error_code: "NOT_A_PLUS_MEMBER" };
  }
  if (!res.ok) {
    throw new Error((data as { error?: string })?.error ?? "Failed to generate cover letter");
  }
  return { id: (data as { id?: string | number })?.id ?? "" };
}

/**
 * Update an existing cover letter's content.
 * PUT /api/application-assets/coverletter/{id}/update/
 */
export async function updateCoverLetter(params: UpdateCoverLetterParams): Promise<string> {
  const res = await authedFetch(`/api/application-assets/${ASSET_TYPE}/${params.id}/update/`, {
    method: "PUT",
    body: JSON.stringify({ content: params.content }),
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error((data as { error?: string })?.error ?? "Failed to update cover letter");
  }
  return params.content;
}

/**
 * Delete a cover letter.
 * DELETE /api/application-assets/coverletter/{id}/delete/
 */
export async function deleteCoverLetter(id: string | number): Promise<void> {
  const res = await authedFetch(`/api/application-assets/${ASSET_TYPE}/${id}/delete/`, { method: "DELETE" });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error((data as { error?: string })?.error ?? "Failed to delete cover letter");
  }
}
