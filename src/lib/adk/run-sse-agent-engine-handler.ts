...
import { createInternalServerError, createBackendConnectionError } from "./error-utils";
import { ProcessedStreamRequest, formatAgentEnginePayload, logStreamStart, logStreamResponse, SSE_HEADERS } from "./run-sse-common";
...
class JSONFragmentProcessor {
  ...
  processChunk(chunk: string): void {
    ...
    console.log(`📝 [JSON PROCESSOR] Full chunk content:`, JSON.stringify(chunk));
        ...
        try {
          ...
          if (part.text && typeof part.text === "string") {
            ...
            if (!this.sentParts.has(partHash)) {
              console.log(`✅ [JSON PROCESSOR] Found new part (thought: ${part.thought}): ${part.text.substring(0, 100)}...`);
  ...
  private hashPart(part: AgentEngineContentPart): string {
    ...
    return `${textHash}-${part.thought}-${!!part.function_call}-${textHash.length}`;
  }
  ...
  private emitCompletePart(part: AgentEngineContentPart): void {
    console.log(
      ...
      part.text?.substring(0, 200) + (part.text && part.text.length > 200 ? "..." : "")
    );
    ...
    console.log(`✅ [JSON PROCESSOR] Successfully emitted complete part as SSE format`);
  }
  ...
  processCompleteFragment(fragment: AgentEngineFragment): void {
    console.log(`✅ [JSON PROCESSOR] Processing complete fragment for agent: ${fragment.author}`);
    ...
    if (fragment.content?.parts && Array.isArray(fragment.content.parts)) {
      console.log(`🔍 [JSON PROCESSOR] Found ${fragment.content.parts.length} parts in complete fragment`);
      ...
      for (const [index, part] of fragment.content.parts.entries()) {
        if (part.text && typeof part.text === "string") {
          ...
          if (!this.sentParts.has(partHash)) {
            ...
            console.log(`⏭️ [JSON PROCESSOR] Skipping duplicate part ${index + 1}`);
          }
        }
      }
    } else {
      console.log(`⚠️ [JSON PROCESSOR] No content.parts found in complete fragment`);
    }
  ...
  finalize(): void {
    ...
    if (this.buffer.trim()) {
      try {
        ...
        console.error("❌ [JSON PROCESSOR] Failed to parse remaining buffer on finalize:", this.buffer, error);
      }
    }
  }
}
 ...
 */
export async function handleAgentEngineStreamRequest(requestData: ProcessedStreamRequest): Promise<Response> {
  console.log("🚀🚀🚀 [AGENT ENGINE] NEW JSON FRAGMENT HANDLER STARTING 🚀🚀🚀");
  ...
  try {
    ...
    logStreamResponse(response.status, response.statusText, response.headers, "agent_engine");
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error("An error occurred while trying to read the error response body from Agent Engine:", error);
      }
      return createBackendConnectionError("agent_engine", response.status, response.statusText, errorDetails);
    }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        console.log("🚀🚀🚀 [AGENT ENGINE] Starting JSON fragment processing 🚀🚀🚀");
        console.log("📋 [AGENT ENGINE] This is the NEW handler with JSONFragmentProcessor");
        ...
        try {
          ...
          const pump = async (): Promise<void> => {
            ...
            if (value) {
              ...
              console.log(`⏰ [STREAMING] Received chunk at ${new Date().toISOString()}, size: ${chunk.length} bytes`);
    ...
    if (error instanceof TypeError && error.message.includes("fetch")) {
      return createBackendConnectionError("agent_engine", 500, "Connection failed", "Failed to connect to Agent Engine");
    }

    return createInternalServerError("Failed to process Agent Engine streaming request", error);
  }
}
