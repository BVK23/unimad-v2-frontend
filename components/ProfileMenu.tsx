...
const ProfileMenu: React.FC<ProfileMenuProps> = ({ isDarkMode, toggleTheme }) => {
                ...
                <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-[#161616] rounded-2xl shadow-xl border border-slate-100 dark:border-white/5 overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-50">
                    ...
                    <div className="p-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                        <div className="flex items-center gap-3 mb-3">
                            <div className="w-12 h-12 rounded-full bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 flex items-center justify-center font-medium text-lg">
                                AB
                            </div>
                            <div className="overflow-hidden">
                                <h3 className="font-medium text-slate-900 dark:text-white truncate">Abhi</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">abhi@example.com</p>
                            </div>
                        </div>
                    ...
                    <div className="p-2 space-y-1">
                        ...
                        >
                            ...
                            <div className={`w-10 h-5 rounded-full relative transition-colors ${isDarkMode ? 'bg-brand-500' : 'bg-slate-200'}`}>
                                <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all shadow-sm ${isDarkMode ? 'left-6' : 'left-1'}`}></div>
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="p-2 border-t border-slate-100 dark:border-white/5">
                        <button className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 text-sm font-medium text-red-600 dark:text-red-400 transition-colors">
                            <LogOut size={18} />
                            Logout
                        </button>
                    </div>
