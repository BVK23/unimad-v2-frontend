...
type UnicoachPlanPaymentModalProps = {
  open: boolean;
  onClose: () => void;
  planId?: UnicoachProductPlanId;
};

export function UnicoachPlanPaymentModal({ open, onClose, planId = "unicoach_program" }: UnicoachPlanPaymentModalProps) {
  const router = useRouter();
  const { isAuthenticated } = useAuthStatus();
  const plan = getUnicoachPlan(planId);
  const [couponOpen, setCouponOpen] = useState(false);
  const [couponInput, setCouponInput] = useState("");

  const { basePrice, finalPrice, appliedCode, discountAmount, isProcessing, isValidatingCoupon, applyCoupon, startCheckout, clearCoupon } =
    useUnicoachRazorpayCheckout({ isAuthenticated, planId });
  ...
  const payAmount = discountAmount > 0 ? finalPrice : basePrice;
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="unicoach-payment-title"
      >
        <button
          type="button"
        ...
        <div key={planId} className="masterclass-onboarding-modal relative z-10 w-full max-w-[440px] rounded-t-[18px] sm:rounded-[14px]">
          <button
            type="button"
            onClick={isProcessing ? undefined : onClose}
          ...
          <div className="px-6 pb-6 pt-8 sm:px-8 sm:pb-8 sm:pt-9">
            <h2 id="unicoach-payment-title" className="text-[22px] font-medium tracking-[-0.44px] text-[#eaeaea]">
              Unicoach {plan.label}
            </h2>
            ...
            <div className="mt-6 rounded-[12px] border border-white/10 bg-white/[0.03] p-5">
              ...
              >
                <span className="relative z-10 px-2">{isProcessing ? "Opening payment…" : `Pay £${payAmount}`}</span>
              </button>
            </div>

            <p className="mt-4 text-center text-[11px] tracking-[-0.22px] text-[#eaeaea]/45">{plan.tagline}</p>
          </div>
        </div>

        {couponOpen && plan.allowsCoupon ? (
          <div
