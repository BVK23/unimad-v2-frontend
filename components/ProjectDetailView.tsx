...
import { ArrowLeft, Trash2, GripVertical, MoveHorizontal, Edit3, Eye, Copy, Type, Image as ImageIcon, Link as LinkIcon, FileText, Plus, Table2, Code2, Figma } from 'lucide-react';
...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({
    ...
    const blocks: PortfolioItem[] = (project.detailedBlocks || []).map((b) => {
        ...
        return {
            ...
            colStart: b.colStart ? Math.max(1, Math.min(gridColumns, Math.round(b.colStart))) : b.colStart,
        };
    });
    ...
    const handleDragStart = (e: React.DragEvent, index: number, blockId: string) => {
        ...
        handleUpdateBlocks(blocks.map((block) => {
    ...
    const handleCoverUpload = (file: File) => {
        ...
        reader.onload = () => {
            onUpdateProject({ ...project, content: String(reader.result || '') });
        };
    ...
    const onDropCover = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (file && isEditMode) handleCoverUpload(file);
    };
        ...
        <div className="flex-1 bg-slate-50 dark:bg-[#080808] h-full overflow-y-auto no-scrollbar relative animate-in slide-in-from-right duration-300">
            ...
            {!hideToolbar && (
                <div className="sticky top-0 z-40 bg-white/80 dark:bg-[#080808]/80 backdrop-blur-md border-b border-slate-100 dark:border-white/5 py-4 px-6 flex items-center justify-between shadow-sm">
                    <button onClick={onBack} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors text-sm font-bold bg-slate-100 dark:bg-white/5 px-4 py-2 rounded-full">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            {isEditMode ? <><Eye size={14} /> Preview Mode</> : <><Edit3 size={14} /> Edit Page</>}
                        </button>
                    </div>
                </div>
            )}
            ...
            >
                ...
                {project.content ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center border-b border-slate-200 dark:border-white/10 border-dashed">
                        <span className="text-slate-300 font-bold uppercase tracking-widest text-sm flex items-center gap-2"><ImageIcon size={20}/> No Cover Image</span>
                    </div>
                )}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#080808] via-[#080808]/20 to-transparent flex flex-col justify-end p-8 md:p-16">
                    <div className={`${editorMaxWidth} mx-auto w-full group/text`}>
                        ...
                        {isEditMode ? (
                            ...
                            <h1 className="text-4xl md:text-6xl font-black text-white mb-2 leading-tight drop-shadow-md">{project.title || 'Untitled Page'}</h1>
                        )}
                ...
                >
                    {blocks.map((block, index) => (
                        <div
                            ...
                            onMouseUpCapture={() => { dragHandleArmedBlockIdRef.current = null; }}
                        >
                            {/* Block Controls */}
                            {isEditMode && (
                                <div className="absolute -left-6 top-1/2 -translate-y-1/2 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity flex flex-col gap-2 z-30">
                                    <div
                                        ...
                                        onDragEnd={resetDragState}
                                        onMouseUp={() => { dragHandleArmedBlockIdRef.current = null; }}
                                    >
                    ...
                    {/* Editor Inline Menu */}
                    {isEditMode && (
                        <div className={`col-span-1 ${gridColumns === 12 ? 'md:col-span-12' : 'md:col-span-3'} mt-8 border-t border-slate-100 dark:border-white/10 pt-8`}>
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                <Plus size={14} /> Add Block
                            </h4>
                            <div className="flex flex-wrap gap-3">
                                {allowedTypes.includes('text') && (
                                    <button onClick={() => handleAddBlock('text')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                        <Type size={16} /> Text
                                    </button>
                                )}
                                {allowedTypes.includes('media') && (
                                    <button onClick={() => handleAddBlock('media')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                        <ImageIcon size={16} /> Media
                                    </button>
                                )}
                                {allowedTypes.includes('page-card') && (
                                    <button onClick={() => handleAddBlock('page-card')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                        <FileText size={16} /> Page
                                    </button>
                                )}
                                {allowedTypes.includes('link-box') && (
                                    <button onClick={() => handleAddBlock('link-box')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                        <LinkIcon size={16} /> Link
                                    </button>
                                )}
                                {allowedTypes.includes('table') && (
                                    <button onClick={() => handleAddBlock('table', { title: 'Table', content: JSON.stringify([['Header 1', 'Header 2'], ['', ''], ['', '']]) })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                        <Table2 size={16} /> Table
                                    </button>
                                )}
                                {allowedTypes.includes('embed') && (
                                    <>
                                        <button onClick={() => handleAddBlock('embed', { title: 'Embed Code', variant: 'code' })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <Code2 size={16} /> Embed Code
                                        </button>
                                        <button onClick={() => handleAddBlock('embed', { title: 'Figma Embed', variant: 'figma' })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
            ...
            {contextMenu && isEditMode && (
                ...
                >
                    {contextMenu.targetId && (
                        <div className="py-1">
                            <button onClick={() => { handleDuplicateBlock(contextMenu.targetId!); setContextMenu(null); }} className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 flex items-center gap-2 font-medium">
                                <Copy size={16} /> Duplicate Block
                            </button>
                            <button onClick={() => { handleRemoveBlock(contextMenu.targetId!); setContextMenu(null); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2 font-medium">
