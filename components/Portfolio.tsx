...
import {
    Edit3, Layout, Image as ImageIcon, Type, Link as LinkIcon,
    Video, GripVertical, Trash2,
    Briefcase, MapPin, Globe, Plus, ExternalLink, Mail, Phone, AlignCenter, AlignLeft,
    ArrowRight, ArrowLeft, Save, Eye, EyeOff, FileText, Copy, GraduationCap, ChevronDown, ChevronRight, Table2, Code2, Figma
} from 'lucide-react';
...
const INITIAL_PROFILE: UserProfile = {
    ...
    education: [
        { id: '1', degree: 'B.Des in Interaction Design', school: 'CCA', year: '2015 - 2019' },
    ],
    contactButtons: [
...
const INITIAL_ITEMS: PortfolioItem[] = [
    ...
    },
    {
        id: '2',
        type: 'text',
        content: "I believe that good design is invisible. It should solve problems without drawing attention to itself. Currently focused on building modular ecosystems.",
        span: 1,
        fontSize: 'base'
    },
    {
...
const Portfolio: React.FC<PortfolioProps> = ({ initialData, onBack }) => {
    const portfolioId = initialData?.id ?? 'portfolio-draft';
    ...
    const [selectedCoverCropRatio, setSelectedCoverCropRatio] = useState<NonNullable<UserProfile['coverCropRatio']>>(profile.coverCropRatio || '16:9');
    ...
    const persistDraft = () => {
        savePortfolioDraft(portfolioId, buildPortfolioData());
    };
    ...
    useEffect(() => {
        ...
        const synced = items.find((item) => item.id === selectedProject.id);
        if (!synced) {
            setSelectedProject(null);
            return;
        }
        if (synced !== selectedProject) setSelectedProject(synced);
    ...
    }, [items.length, profile.itemsAboveProfileCount]);
    ...
    const handlePublish = () => {
        ...
        if (publishedUrl) {
            showToast('Changes saved and published');
            setShowPublishMenu(false);
            return;
        }
        ...
        savePublishedUrl(portfolioId, trimmed);
        showToast('Published');
        setShowPublishMenu(false);
    };

    const handleCopyPublishedLink = async () => {
        const url = (publishedUrl ?? publishUrlInput).trim();
        if (!url) {
            showToast('Enter a URL to copy');
            return;
        }
        try {
    ...
    const publishMenuDropdown = showPublishMenu ? (
        <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(20rem,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl dark:border-white/10 dark:bg-[#0b0b0b]">
            ...
            <label className="mt-3 block text-[10px] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400">
                URL
            </label>
            <input
                ...
                placeholder="e.g. alexmorgan.unimad.io"
            ...
            <div className="mt-3 flex items-center gap-2">
                ...
                {isEditMode && (
                    <button
                        ...
                        className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-full bg-brand-600 px-3 py-2 text-xs font-semibold text-white shadow-md shadow-brand-500/20 transition-all hover:scale-[1.02] active:scale-95"
                    >
    ...
    const applyCoverCrop = (
        ...
        const img = new Image();
        img.onload = () => {
            const { cropWidth, cropHeight, offsetX, offsetY } = getCoverCropGeometry(
                img.naturalWidth,
                img.naturalHeight,
                ratioLabel,
                zoom,
    ...
    const coverCropGeometry = coverCropModal && coverCropImageSize.width && coverCropImageSize.height
    ...
    const coverCropPreviewStyle = coverCropGeometry && coverCropImageSize.width && coverCropImageSize.height ? {
    ...
    const duplicateItem = (id: string) => {
        ...
        if (original) {
            const newItem = { ...original, id: Date.now().toString() };
    ...
    useEffect(() => {
        ...
        const updateGridMetrics = () => {
            ...
            setGridMetrics((prev) => (
                prev.rowHeight === GRID_ROW_PX && prev.rowGap === rowGap
                    ? prev
                    : { rowHeight: GRID_ROW_PX, rowGap }
            ));
        };
    ...
    const getRowSpanForItem = (item: PortfolioItem) => {
        ...
        const heightPx = item.type === 'text' && item.isCollapsible && item.isCollapsed
    ...
    const updateContactButton = (id: string, updates: Partial<ContactButton>) => {
        setProfile((prev) => ({
            ...prev,
            contactButtons: (prev.contactButtons || []).map((button) => 
    ...
    const itemsAboveProfile = Math.min(
        items.length,
        Math.max(0, profile.itemsAboveProfileCount ?? 0)
    );
    ...
    const resolveDropSourceIndex = (): number => {
        ...
        if (draggingIdRef.current) {
            const i = items.findIndex((it) => it.id === draggingIdRef.current);
            if (i !== -1) return i;
        }
        ...
        <>
            {selectedProject ? (
                <ProjectDetailView
                    ...
                    onUpdateProject={(updated) => { updateItemContent(updated.id, updated); setSelectedProject(updated); }}
                />
            ) : (
                <div className="flex-1 bg-slate-50 dark:bg-[#080808] h-full overflow-y-auto no-scrollbar relative">
                    ...
                    {isEditMode && profile.showCover === false ? (
                        <div className="max-w-5xl mx-auto px-4 mt-6">
                            <button onClick={() => setProfile({...profile, showCover: true})} className="w-full py-4 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-xl text-slate-400 hover:text-brand-600 hover:border-brand-500 transition-colors flex items-center justify-center gap-2 text-sm font-medium">
                                <ImageIcon size={16} /> Show Cover Image
                            </button>
                        </div>
                    ) : (profile.showCover !== false && (
                        <div className="max-w-5xl mx-auto px-4 mt-6">
                            <div className={`h-40 md:h-48 w-full relative group bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 overflow-hidden ${
                                ...
                                {isEditMode && (
                                    <div className="absolute inset-0 bg-black/40 hidden group-hover:flex items-center justify-center gap-4 backdrop-blur-sm transition-all z-10">
                                        ...
                                        <button onClick={() => setProfile({...profile, showCover: false})} className="bg-red-500 text-white p-2.5 rounded-full shadow-xl hover:scale-105 active:scale-95 transition-transform" title="Hide Cover">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                )}
                    ...
                    <div className="max-w-5xl mx-auto px-4 pb-40">
                        ...
                        {showProfileSection && itemsAboveProfile > 0 && (
                            ...
                            >
                                {items.slice(0, itemsAboveProfile).map((item, sliceIndex) => {
                                ...
                                <motion.div
                                    ...
                                    onMouseUpCapture={() => { dragHandleArmedItemIdRef.current = null; }}
                                    className={`
                                    ...
                                    {isEditMode && (
                                        <div className="absolute -left-6 top-1/2 -translate-y-1/2 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity flex flex-col gap-2 z-30">
                                            <div
                                                ...
                                                onDragEnd={handleDragEnd}
                                                onMouseUp={() => { dragHandleArmedItemIdRef.current = null; }}
                                            >
                        ...
                        {showProfileSection && (
                        <div className={`relative mb-16 flex flex-col bg-white dark:bg-[#080808] border border-slate-200 dark:border-white/5 border-t-0 rounded-b-2xl px-6 md:px-10 pb-12 ${
                            profile.showCover !== false ? 'pt-0' : 'pt-10'
                        } ${
                            profile.profileAlignment === 'center' ? 'items-center text-center' : 
                            profile.profileAlignment === 'right' ? 'items-end text-right' : 
                            ...
                            {isEditMode && profile.showAvatar === false ? (
                                ...
                                <div className={`relative group ${profile.showCover !== false ? '-mt-16 md:-mt-20' : 'mt-0'} z-10 flex-shrink-0`}>
                                    <img src={profile.avatarUrl} className="w-32 h-32 md:w-36 md:h-36 rounded-full border-4 border-white dark:border-[#080808] shadow-lg object-cover bg-white dark:bg-[#080808]" />
                                    ...
                                    {isEditMode && (
                                        <>
                                            ...
                                            <div className="absolute inset-0 rounded-full bg-black/60 hidden group-hover:flex flex-col items-center justify-center gap-3 cursor-pointer backdrop-blur-sm overflow-hidden p-2 z-10">
                                                ...
                                                <div onClick={(e) => { e.stopPropagation(); setProfile({...profile, showAvatar: false}); }} className="flex items-center justify-center gap-2 text-red-400 hover:text-red-500 transition-colors">
                                ...
                                {isEditMode ? (
                                    <div className="space-y-1">
                                        <input 
                                            ...
                                            className={`text-[32px] md:text-[40px] font-semibold tracking-tight text-slate-900 dark:text-white bg-transparent outline-none w-full ${
                                                profile.profileAlignment === 'center' ? 'text-center' : 
                                                profile.profileAlignment === 'right' ? 'text-right' : 'text-left'
                                            }`}
                                    ...
                                    <>
                                        <div className="flex flex-col gap-1.5">
                                            {profile.name && <h1 className="text-[32px] md:text-[40px] font-semibold tracking-tight leading-none text-slate-900 dark:text-white">{profile.name}</h1>}
                                            {profile.tagline && <p className="text-[12px] md:text-[15px] leading-tight text-slate-500 dark:text-slate-400 font-medium">{profile.tagline}</p>}
                                        </div>
                                    </>
                                )}
                            </div>
                            ...
                            <div className="w-full max-w-2xl mt-10">
                                <div className={`flex flex-wrap gap-4 ${
                                    profile.profileAlignment === 'center' ? 'justify-center' :
                                    profile.profileAlignment === 'right' ? 'justify-end' :
                                    'justify-start'
                                }`}>
                                    {visibleContactButtons.length > 0 ? visibleContactButtons.map((button) => (
                                        <a
                                            key={button.id}
                        ...
                        {showProfileSection && isEditMode && itemsAboveProfile > 0 && (
                            <div
                                ...
                                className="mb-6 flex min-h-[36px] cursor-default items-center justify-center rounded-xl border border-dashed border-slate-200 text-[10px] font-medium uppercase tracking-wider text-slate-400 transition-colors hover:border-brand-400/60 hover:text-brand-600 dark:border-white/10 dark:text-slate-500 dark:hover:border-brand-400/50 dark:hover:text-brand-400"
                            >
                            ...
                            {isEditMode && (
                                <div className="col-span-1 md:col-span-12 mt-8">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        <button onClick={() => addItem('text')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <Type size={16} /> Text
                                        </button>
                                        <button onClick={() => addItem('media')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <ImageIcon size={16} /> Media
                                        </button>
                                        <button onClick={() => addItem('page-card')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <FileText size={16} /> Page
                                        </button>
                                        <button onClick={() => addItem('link-box')} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <LinkIcon size={16} /> Link
                                        </button>
                                        <button onClick={() => addItem('table', { title: 'Table', content: JSON.stringify([['Header 1', 'Header 2', 'Header 3'], ['', '', ''], ['', '', '']]) })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <Table2 size={16} /> Table
                                        </button>
                                        <button onClick={() => addItem('embed', { title: 'Embed Code', variant: 'code' })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                                            <Code2 size={16} /> Embed Code
                                        </button>
                                        <button onClick={() => addItem('embed', { title: 'Figma Embed', variant: 'figma' })} className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 hover:text-brand-600 hover:border-brand-500 transition-all font-medium text-sm flex items-center gap-2 bg-white dark:bg-[#111] shadow-sm">
                    ...
                    {contextMenu && isEditMode && (
                        ...
                        >
                            {contextMenu.targetId && (
                                // Block Options
                                <div className="py-1">
                                    <button onClick={() => { duplicateItem(contextMenu.targetId!); closeContextMenu(); }} className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 flex items-center gap-2">
                                        <Copy size={16} /> Duplicate Block
                                    </button>
                                    <button onClick={() => { removeItem(contextMenu.targetId!); closeContextMenu(); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2">
                    ...
                    {coverCropModal && (
                        <div className="fixed inset-0 z-[220] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setCoverCropModal(null)}>
                            <div className="w-full max-w-3xl bg-white dark:bg-[#0c0c0c] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/10">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Choose a crop ratio, zoom, and position for the cover image.</p>
                                    </div>
                                    <button onClick={() => setCoverCropModal(null)} className="p-2 rounded-full text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors">
                                        Close
                                    </button>
                                </div>
