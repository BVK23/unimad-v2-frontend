  ...
  return (
    <div className={`relative h-full w-full overflow-hidden bg-slate-100 dark:bg-slate-900 ${className}`} aria-hidden>
      ...
      <div className="h-[32%] shrink-0 bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900 px-2 pb-2 pt-2">
        ...
        {project.description ? <p className="mt-0.5 line-clamp-1 text-[5px] text-white/70">{project.description}</p> : null}
      </div>
      ...
      <div className="grid h-[68%] grid-cols-12 gap-0.5 p-1">
        {blocks.length > 0 ? (
          blocks.slice(0, 4).map(block => {
            ...
            const preview =
              block.type === "link-box" ? block.title || stripText(block.content || "Link") : stripText(block.content || block.title || "");
            ...
            return (
              ...
              >
                ...
                <p className="line-clamp-3 px-0.5 py-px text-[4px] leading-[1.15] text-slate-600 dark:text-slate-300">{preview || "…"}</p>
              </div>
            );
          })
        ) : (
