...
const VpdPreview: React.FC<VpdPreviewProps> = ({ project, onOpenEditor, variant = 'panel' }) => {
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className={`relative min-h-0 ${isPanel ? 'flex flex-1 flex-col' : ''} ${
                        hasContent ? '' : 'min-h-[160px]'
                    }`}
                >
                    {onOpenEditor && (
                        <div
                    ...
                    {hasContent ? (
                        <div
                            className={`pointer-events-none overflow-y-auto p-4 ${
                                isPanel ? 'min-h-0 flex-1' : 'max-h-[280px]'
                            }`}
                        >
                            {project.title && (
                                <h4 className="mb-3 text-sm font-semibold text-slate-900 dark:text-white">{project.title}</h4>
                            )}
                        ...
                        <div className="pointer-events-none flex min-h-[160px] flex-col items-center justify-center px-4 py-8 text-center">
                            <p className="text-xs text-slate-400">
                                Generate a draft or pick a template to see a preview here.
                            </p>
                        </div>
                    )}
                </div>
