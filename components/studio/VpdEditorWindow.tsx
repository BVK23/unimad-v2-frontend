...
const VpdEditorWindow: React.FC<VpdEditorWindowProps> = ({ project, onClose, onUpdateProject }) => {
    return (
        <div className="fixed inset-0 z-[60] flex flex-col bg-slate-100 dark:bg-[#050505] animate-in fade-in duration-200">
            <div className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 dark:border-slate-800 dark:bg-[#0a0a0a]">
            ...
            <div className="flex shrink-0 border-b border-slate-200 bg-white px-5 py-2 dark:border-slate-800 dark:bg-[#0a0a0a] sm:hidden">
                <VpdExportActions project={project} />
            </div>

            <div className="flex min-h-0 flex-1 justify-center overflow-hidden px-4 py-6 md:px-8">
                <div className="h-full w-full max-w-5xl overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xl dark:border-slate-800 dark:bg-[#080808]">
                    <ProjectDetailView
                        project={project}
                        onBack={onClose}
