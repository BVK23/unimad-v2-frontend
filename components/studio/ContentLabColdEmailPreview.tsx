...
interface ContentLabColdEmailPreviewProps {
  draft: ColdEmailAsset | null;
  onCopy: () => void;
...
export default function ContentLabColdEmailPreview({
  draft,
  onCopy,
  copyFeedback,
  onEditorActivate,
  ...
  isGenerating = false,
}: ContentLabColdEmailPreviewProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg w-full max-w-xl mx-auto flex flex-col min-h-[600px]">
      ...
      <div className={`${isGenerating ? "flex-1 overflow-hidden" : "flex-1 p-6 md:p-12 overflow-y-auto"}`}>
        {isGenerating ? (
          <AssetPreviewLoadingOverlay label="Generating cold email..." />
        ) : hasContent ? (
          <CoverLetterInlineEditor
            key={String(draft?.id ?? "cold-email-empty")}
            value={content}
          ...
          <p className="text-slate-400 dark:text-slate-600 font-serif">Your cold email draft will appear here....</p>
        )}
      </div>
    </div>
  );
