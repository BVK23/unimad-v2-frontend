...
import {
    ...
    Video, FolderPlus, GripVertical, Trash2, X, MoveHorizontal,
    ExternalLink, Plus, ArrowRight, PlayCircle, FileText
} from 'lucide-react';
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            setItems(prev => prev.map(item =>
                item.id === resizing.id && item.span !== newSpan
                    ? { ...item, span: newSpan as 1 | 2 | 3 }
                    : item
            ));
        };
        ...
        <div className="flex-1 h-full relative flex flex-col">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111]">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        {isEditMode ? <><Layout size={14} /> Done</> : <><Edit3 size={14} /> Edit</>}
                    </button>
                </div>
            </div>
                            ...
                            >
                                ...
                                {isEditMode && (
                                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white shadow-md rounded-full p-1 z-30">
                                        <button onClick={() => removeItem(item.id)} className="p-1 rounded-full hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                                    </div>
                                )}
                                ...
                                <div className="h-full">
                                    ...
                                    {item.type === 'image' && (
                                        <div className="h-64 bg-slate-100 dark:bg-slate-900">
                                            <img src={item.content} className="w-full h-full object-cover" alt="VPD Asset" />
                                        </div>
                                    )}

                                    {item.type === 'project' && (
                                        <div className="h-64 relative group/card cursor-pointer">
                                            <img src={item.content} className="w-full h-full object-cover" alt={item.title} />
                                ...
                                {isEditMode && (
                                    <>
                                        <div className="absolute left-2 top-2 text-slate-400 cursor-grab opacity-0 group-hover:opacity-100 p-1 bg-white/80 rounded" title="Drag">
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white dark:bg-[#1a1a1a] shadow-xl border border-slate-200 dark:border-slate-800 rounded-full px-4 py-2 flex items-center gap-2 z-40">
                <button onClick={() => addItem('text')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors" title="Add Text">
                    <Type size={20} />
                </button>
                <button onClick={() => addItem('image')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors" title="Add Image">
                    <ImageIcon size={20} />
                </button>
                <button onClick={() => addItem('project')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors" title="Add Project">
                    <FolderPlus size={20} />
                </button>
                <button onClick={() => addItem('video')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors" title="Add Video">
                    <Video size={20} />
                </button>
                <button onClick={() => addItem('link')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors" title="Add Link">
                    <LinkIcon size={20} />
                </button>
            </div>
        </div>
