    ...
    >
      <div className="flex">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-1.5 relative z-10 w-full overflow-hidden text-ellipsis whitespace-nowrap">
            ...
            <span className="flex items-center gap-1">
              <span className="flex items-center gap-2">
                ...
                <div
                  className="relative w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden cursor-pointer"
                  onClick={() => onViewProfile(post.author.id)}
                >
                  {post.author.avatarUrl && <Image src={post.author.avatarUrl} alt="" fill sizes="20px" className="object-cover" />}
                </div>
          ...
          {/* Media / Ad Image */}
          {post.imageUrl && (
            <div className="mb-2 rounded-lg overflow-hidden border border-slate-100 dark:border-white/5 bg-slate-900 relative w-full aspect-video max-h-[460px]">
              <Image
                src={post.imageUrl}
                alt="Post content"
                fill
