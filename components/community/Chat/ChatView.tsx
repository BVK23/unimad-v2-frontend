...
import { Search, MoreVertical, Phone, Video, Send, ArrowLeft, Image, Smile, Paperclip } from 'lucide-react';
...
const ChatView: React.FC<ChatViewProps> = ({ initialUserId, onBack }) => {
    ...
    const startChat = initialUserId
        ? (MOCK_CHATS.find(c => c.user.id === initialUserId) ?? null)
        : null;
    ...
    return (
        <div className="bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm h-[calc(100vh-140px)] min-h-[600px] flex">
            ...
            <div className={`w-full md:w-72 border-r border-slate-200 dark:border-white/5 flex flex-col ${selectedChat ? 'hidden md:flex' : 'flex'}`}>
                {/* Header */}
                <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center gap-3">
                    ...
                    <div className="flex-1 flex justify-between items-center">
                        ...
                        <button className="text-slate-400 hover:text-brand-500"><MoreVertical size={20} /></button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {MOCK_CHATS.map(chat => (
                        <div
                            ...
                            className={`p-3 mx-2 rounded-lg flex gap-3 cursor-pointer transition-colors ${selectedChat?.id === chat.id
                        ...
                        >
                            <div className="relative">
                                ...
                                {chat.user.status === 'online' && <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white dark:border-[#1a1a1a] rounded-full"></span>}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline mb-0.5">
                                    <h3 className={`text-sm font-medium truncate ${selectedChat?.id === chat.id ? 'text-brand-700 dark:text-brand-400' : 'text-slate-900 dark:text-white'}`}>
                                        {chat.user.name}
                                    </h3>
                                    <span className="text-[10px] text-slate-400">{chat.lastMessage.timestamp}</span>
                                </div>
                                <p className={`text-xs truncate ${chat.unreadCount > 0 ? 'font-medium text-slate-800 dark:text-slate-200' : 'text-slate-500'}`}>
                ...
                <div className="flex-1 flex flex-col bg-slate-50 dark:bg-[#0a0a0a]">
                    ...
                    <div className="h-16 px-4 bg-white dark:bg-white/5 border-b border-slate-200 dark:border-white/5 flex items-center justify-between shadow-sm z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <img src={selectedChat.user.avatar} alt="" className="w-9 h-9 rounded-full object-cover" />
                            <div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        ...
                        {selectedChat.messages.map((msg, index) => {
                            ...
                            return (
                                <div key={msg.id} className={`group flex gap-3 px-2 py-1 hover:bg-slate-100/50 dark:hover:bg-white/5 -mx-2 rounded-lg transition-colors ${showHeader ? 'mt-4' : 'mt-0'}`}>
                                    {/* Avatar Column */}
                                        ...
                                        {showHeader && (
                                            <div className="flex items-baseline gap-2 mb-0.5">
                                                <span className="text-sm font-medium text-slate-900 dark:text-white">
                                                    {isMe ? 'You' : selectedChat.user.name}
                                                </span>
                                                <span className="text-[10px] text-slate-400 font-medium">{msg.timestamp}</span>
                                            </div>
                                        )}
                                        <p className="text-sm text-slate-800 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
                                            {msg.content}
                                        </p>
                                    </div>
                                </div>
