  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm">
        {/* Cover Info */}
        <div className="h-32 md:h-48 bg-slate-200 dark:bg-slate-800 relative overflow-hidden">
          {author.bannerUrl ? (
            <Image src={author.bannerUrl} alt="Cover" fill sizes="(max-width: 768px) 100vw, 1024px" className="object-cover" />
          ) : (
            <div className="w-full h-full bg-gradient-to-r from-brand-600 to-brand-600"></div>
          )}
        </div>
        ...
        <div className="px-6 pb-6 relative">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end -mt-10 mb-4 gap-4 md:gap-0">
            <div className="w-32 h-32 rounded-full p-1.5 bg-white dark:bg-slate-950">
              <div className="w-full h-full rounded-full bg-slate-100 overflow-hidden relative">
                {author.avatarUrl ? (
                  <Image src={author.avatarUrl} alt={author.name} fill sizes="128px" className="object-cover" />
                ) : (
              ...
              <div className="relative">
                ...
                {showMenu && (
                  <>
                    ...
                    <div className="absolute top-full mt-2 left-0 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 py-1 z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 py-3 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-white/5 px-3 py-1.5 rounded-lg w-fit">
                ...
                <span>
                  You both follow <span className="font-medium">Design Systems</span> and <span className="font-medium">UX Research</span>
                </span>
              </div>
      ...
      <div className="space-y-4">
        {userPosts.length > 0 ? (
          userPosts.map(post => <PostCard key={post.id} post={post} onViewProfile={() => {}} />)
        ) : (
