...
type ActionHintTooltipProps = {
  ...
  /** When false, renders children only. */
  enabled: boolean;
  message: string;
  ctaLabel?: string;
  onCta?: () => void;
...
/**
 * Instant hover/tap popover with info text + optional right-aligned CTA.
 * Positions by absolute top/left (no translateY) and clamps fully inside the viewport.
 */
export function ActionHintTooltip({
  children,
  enabled,
  message,
  ctaLabel,
  ...
  useLayoutEffect(() => {
    ...
    updateCoords();
  }, [open, message, ctaLabel, updateCoords]);
