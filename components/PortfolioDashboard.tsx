...
const PortfolioDashboard: React.FC<PortfolioDashboardProps> = ({ portfolios, onEditPortfolio, onCreatePortfolio, isCreating = false }) => {
  ...
  const handleDuplicate = (e: React.MouseEvent, portfolio: PortfolioData) => {
    ...
    setLocalPortfolios([newPortfolio, ...visiblePortfolios]);
  ...
  const handleDelete = (e: React.MouseEvent, id: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this portfolio?")) {
      setLocalPortfolios(visiblePortfolios.filter(p => p.id !== id));
    }
    setActiveMenuId(null);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          {visiblePortfolios.map(portfolio => (
            ...
            >
              ...
              <div
                className="h-40 relative overflow-hidden flex items-center justify-center rounded-t-xl bg-cover bg-center"
                style={{ backgroundImage: `url(${portfolio.profile.coverUrl})` }}
              >
