...
import type { ProfileExperience } from "@/features/user-profile/types";
import { descriptionsArrayToString, descriptionsStringToArray, formatDateRange } from "@/features/user-profile/utils/profile-format";
import { Pencil, Plus, Trash2 } from "lucide-react";

type ExperienceForm = Omit<ProfileExperience, "descriptions"> & { descriptions: string };
...
export function ProfileExperienceSection() {
  ...
  const items = useMemo(() => (profile?.experiences ?? []) as ProfileExperience[], [profile?.experiences]);
  ...
  const save = async () => {
    const payload: ProfileExperience = {
    ...
    await updateProfile.mutateAsync({ experiences: next });
    setModalOpen(false);
  };

  const remove = async (index: number) => {
    await updateProfile.mutateAsync({ experiences: items.filter((_, i) => i !== index) });
    setDeleteIndex(null);
  };

  const canSave = Boolean(form.organisation.trim() && form.role.trim() && form.startDate && (isPresent || form.endDate));

  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Work experience</h3>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {items.map((exp, index) => (
            <div key={`${exp.organisation}-${index}`} className="group space-y-1">
              <button
                type="button"
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <span className="text-sm font-semibold text-slate-900 dark:text-white">{exp.role}</span>
                  <span className="text-xs text-slate-500">{formatDateRange(exp.startDate, exp.endDate)}</span>
                </div>
                <div className="mt-1 flex flex-wrap items-center justify-between gap-2 text-xs">
                  <span className="font-medium text-brand-600 dark:text-brand-400">{exp.organisation}</span>
                  {exp.location ? <span className="italic text-slate-500">{exp.location}</span> : null}
                </div>
                {exp.descriptions?.length ? (
      ...
      <ProfileFormModal
        open={modalOpen}
        title={editIndex === null ? "Add experience" : "Edit experience"}
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <ProfileFormField label="Company">
            <ProfileFormInput value={form.organisation} onChange={e => setForm(f => ({ ...f, organisation: e.target.value }))} />
          </ProfileFormField>
          <ProfileFormField label="Role">
            <ProfileFormInput value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} />
          </ProfileFormField>
          ...
          <ProfileFormField label="Description" className="sm:col-span-2">
            ...
            />
            <p className="mt-1 text-[11px] text-brand-600 dark:text-brand-400">Start each bullet with &quot;- &quot;</p>
          </ProfileFormField>
        </div>
      </ProfileFormModal>
