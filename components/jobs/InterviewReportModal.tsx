...
const InterviewReportModal: React.FC<InterviewReportModalProps> = ({ session, onClose, onRetake }) => {
    ...
    const mockTranscript = [
        { q: "Tell me about a time you handled a difficult stakeholder.", a: "I once had a PM who pushed for a feature that wasn't user-tested. I set up a quick usability test to gather data...", feedback: "Great use of data to back your decision.", score: 'Good' },
        { q: "How do you prioritize your design tasks?", a: "I use the Eisenhower Matrix...", feedback: "Solid framework, but try to be more specific to product design context.", score: 'Neutral' },
        { q: "Walk me through your portfolio piece.", a: "So, this project was for a fintech app...", feedback: "Good structure, but you started a bit abruptly. Set the context first.", score: 'Good' }
    ];
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-4xl h-[85vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30 p-6 rounded-2xl">
                        ...
                        <p className="text-slate-700 dark:text-slate-300 leading-relaxed text-sm">
                            You demonstrated strong technical knowledge and cultural fit. To increase your hiring probability to the next level, focus on structuring your "Situation" and "Task" sections more concisely in behavioral questions.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            {mockTranscript.map((item, idx) => (
                                <div key={idx} className="group border border-slate-200 dark:border-slate-800 rounded-xl p-5 hover:border-blue-200 dark:hover:border-blue-700 transition-colors bg-white dark:bg-slate-900/50">
                                    <div className="flex justify-between items-start mb-3">
                                        <h4 className="font-medium text-slate-900 dark:text-white text-sm">Q{idx + 1}: {item.q}</h4>
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wider ${item.score === 'Good' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                                            {item.score}
                                        </span>
                                    </div>
                ...
                <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-end gap-3">
                    <button onClick={onClose} className="px-6 py-2.5 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors">
                        Close
                    </button>
                    <button onClick={onRetake} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow-lg shadow-blue-600/20 flex items-center gap-2 transition-all active:scale-95">
