...
import { GeneratorContext, Job, MOCK_JOBS } from '../../types/jobs';
...
const JobsMain: React.FC<JobsMainProps> = ({ onNavigateToStudio }) => {
    ...
    const [trackerJobs, setTrackerJobs] = useState<Job[]>(() =>
        MOCK_JOBS.filter((job) => job.applicationStatus)
    );

    const handleAddToTracker = (job: Job) => {
        setTrackerJobs((prev) => [job, ...prev]);
    };
        ...
        <div className="flex-1 bg-slate-50 dark:bg-[#0a0a0a] h-full overflow-hidden flex flex-col font-sans">
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {activeTab === 'tracker' && (
                    <ApplicationTracker
                        onNavigateToStudio={onNavigateToStudio}
                        onStartInterviewPrep={() => setActiveTab('interview')}
                        jobs={trackerJobs}
                        setJobs={setTrackerJobs}
                    />
                )}
