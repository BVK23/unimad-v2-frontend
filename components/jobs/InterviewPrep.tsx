...
import { Mic, Play, Square, SkipForward, BarChart2, Clock, Calendar, ChevronRight, Volume2, MicOff, MessageSquare, X, Smartphone, Globe, Bot, Trash2 } from 'lucide-react';
                ...
                <div className="bg-[#0B1121] rounded-3xl p-10 text-white mb-10 shadow-2xl relative overflow-hidden group border border-slate-800">
                    ...
                    <div className="relative z-10 max-w-2xl">
                        ...
                        <p className="text-slate-400 mb-8 text-lg leading-relaxed">
                            Practice with our AI interviewer. Get real-time feedback on your answers, pacing, and tone. Tailored to your target job description.
                        </p>
                        <button
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4">
                                <div className="text-right">
                                    <span className={`block text-xl font-medium ${session.score >= 80 ? 'text-green-600' : 'text-orange-500'}`}>{session.score}</span>
            ...
            <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
                <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-2xl rounded-3xl p-8 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button onClick={() => setView('dashboard')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-400 transition-colors">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-6 mb-10">
                        <div>
                            ...
                            <div className="relative">
                                <span className="absolute left-4 top-3.5 text-slate-400"><Globe size={18} /></span>
                                <input
                                    type="text"
                                    value={company}
                    ...
                    <div className="flex gap-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                        <button onClick={() => setView('dashboard')} className="flex-1 py-3.5 bg-white border border-slate-200 dark:bg-slate-800 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-50 transition-colors">
                            Cancel
                        </button>
                        <button
        ...
        return (
            <div className="fixed inset-0 z-50 flex flex-col bg-[#0B1121] text-white">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-30 pointer-events-none">
                    <div className={`w-[600px] h-[600px] bg-blue-600/20 rounded-full blur-[120px] transition-all duration-300 ${isListening ? 'scale-110 opacity-50' : 'scale-100'}`}></div>
                </div>
                ...
                <div className="p-8 flex justify-between items-center relative z-20">
                    ...
                    <button onClick={handleFinish} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-sm font-medium transition-colors">
                        End Interview
                    </button>
                </div>

                <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-8 text-center max-w-4xl mx-auto w-full">
                    <span className="text-blue-400 font-medium tracking-widest text-sm uppercase mb-8">Question {currentQuestionIndex + 1} of {questions.length}</span>
                    ...
                    <div className="w-full max-w-2xl bg-white/5 border border-white/10 rounded-2xl p-8 mb-12 text-left min-h-[120px] backdrop-blur-sm relative">
                        <div className="absolute -top-3 left-6 px-3 py-1 bg-blue-600 rounded-md text-xs font-semibold uppercase tracking-wider shadow-lg">You</div>
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto animate-in fade-in">
            <div className="flex items-center gap-4 mb-8">
                <button onClick={() => setView('dashboard')} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
            ...
            <div className="bg-white dark:bg-[#1a1a1a] p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-8 leading-relaxed">
                    You demonstrated strong problem-solving skills and provided clear examples. However, try to be more concise when describing the "Situation" in your STAR method answers.
                </p>
                    ...
                    {questions.map((q, i) => (
                        <div key={i} className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800/50">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className="px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-medium rounded-full">Good</span>
                            </div>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Strong opening, but the conclusion was a bit rushed. Consider summarizing your impact more clearly.</p>
                        </div>
                    ))}
                </div>
            </div>
