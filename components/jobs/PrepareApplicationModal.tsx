...
import { X, FileText, Mail, MessageSquare, Briefcase, Wand2, Copy, Download, UserSquare2 } from "lucide-react";
import { Job, ContentGeneratorType } from "../../types/jobs";
...
const PrepareApplicationModal: React.FC<PrepareApplicationModalProps> = ({ job, onClose, initialTab = "cover-letter" }) => {
  ...
  const tabs: { id: ContentGeneratorType; label: string; icon: React.ElementType }[] = [
    { id: "cover-letter", label: "Cover Letter", icon: FileText },
    { id: "cold-email", label: "Cold Email", icon: Mail },
    { id: "referral", label: "Referral Request", icon: MessageSquare },
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-4xl h-[600px] rounded-3xl shadow-2xl flex overflow-hidden border border-slate-200 dark:border-slate-800">
        {/* Sidebar */}
        <div className="w-64 bg-slate-50 dark:bg-[#111] border-r border-slate-200 dark:border-slate-800 p-6 flex flex-col">
          <div className="mb-6">
          ...
          <div className="space-y-1">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-3 ${activeTab === tab.id ? "bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700" : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50"}`}
              >
                {tab.label}
              </button>
            ))}
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-[#151515]">
          ...
          <div className="h-16 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-6">
            <h2 className="font-medium text-slate-900 dark:text-white flex items-center gap-2">
              <Wand2 size={16} className="text-blue-500" /> Generate {tabs.find(t => t.id === activeTab)?.label}
            </h2>
            ...
            {!generatedContent ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto p-8 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                  We&apos;ll use details from your resume and the job description to create a tailored{" "}
