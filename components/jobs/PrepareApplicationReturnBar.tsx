...
import { prepareTabLabel, type PrepareApplicationReturnSession } from "@/lib/jobs/prepare-application-return";
import { Save, X } from "lucide-react";

type PrepareApplicationReturnBarProps = {
  session: PrepareApplicationReturnSession;
  /** Tab shown in the banner (may differ from session.tab while switching studio topics). */
  currentTab?: PrepareApplicationReturnSession["tab"];
...
export function PrepareApplicationReturnBar({
  ...
  const tabLabel = prepareTabLabel(currentTab ?? session.tab);
  return (
    <div className="flex h-16 shrink-0 items-center gap-3 border-b border-brand-200/80 bg-brand-50/90 px-4 dark:border-brand-800/50 dark:bg-brand-950/40 sm:px-6">
      <button
        type="button"
        onClick={onDismiss}
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-slate-900 dark:text-white">
          Editing {tabLabel} for {session.role}
        </p>
