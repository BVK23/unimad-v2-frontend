    ...
    <div className="flex max-h-[80vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl animate-in zoom-in-95 duration-200 dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-slate-100 p-5 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-md bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-700 dark:bg-brand-900/40 dark:text-brand-300">
            {posts.length}
          </span>
        </div>
        <button
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50/40 p-4 dark:bg-slate-900">
        {posts.length > 0 ? (
          posts.map(post => <LinkedInPostListCard key={post.id} post={post} onClick={() => onPostClick(post)} onDelete={onDeletePost} />)
        ) : (
