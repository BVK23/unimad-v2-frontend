        ...
        <div className="aspect-[16/10] w-full bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center p-12 overflow-hidden relative group">
          {calendlyUrl ? (
            <div className="w-full h-full flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-slate-500">Calendly Scheduler would load here for: <br/><span className="font-mono text-xs">{calendlyUrl}</span></p>
              <button className="px-8 py-3 bg-brand-600 text-white rounded-full font-medium shadow-lg">
                Schedule Meeting
              </button>
            </div>
          ) : (
        ...
        <div className="p-12 lg:p-20 lg:w-1/2 space-y-12">
          <div className="space-y-4">
            ...
            <p className="text-lg text-white/60 leading-relaxed">
              {description || "I'm currently available for freelance projects and full-time opportunities. Reach out and let's start a conversation."}
            </p>
          </div>

          <div className="space-y-6">
            <div className="flex items-center gap-4 text-white/80">
              <div className="p-3 bg-white/10 rounded-2xl"><Mail size={20} /></div>
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" className="w-full px-4 py-3 bg-slate-50 border-transparent focus:bg-white focus:border-brand-500 rounded-xl outline-none transition-all" placeholder="Alex Morgan" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Email Address</label>
                <input type="email" className="w-full px-4 py-3 bg-slate-50 border-transparent focus:bg-white focus:border-brand-500 rounded-xl outline-none transition-all" placeholder="alex@example.com" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Message</label>
              <textarea rows={4} className="w-full px-4 py-3 bg-slate-50 border-transparent focus:bg-white focus:border-brand-500 rounded-xl outline-none transition-all resize-none" placeholder="How can I help you?" />
            </div>
