...
type ResumeOptionsFormProps = {
  /** Called with the list of extracted sections (e.g. ["educations", "skills"]) or [] when answering manually. */
  onComplete: (extractedSections: string[]) => void;
};
...
export default function ResumeOptionsForm({ onComplete }: ResumeOptionsFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setIsOpen(false);
      setFile(null);
      onComplete(Array.isArray(response?.extracted_sections) ? response.extracted_sections : []);
    } catch (err) {
        ...
        >
          <div className="relative w-full max-w-md rounded-[14px] bg-white border border-[rgba(12,15,26,0.07)] shadow-[0_8px_32px_rgba(52,109,224,0.18)] p-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <PrimaryButton onClick={handleSubmit} disabled={!file} fullWidth>
                Upload
              </PrimaryButton>
            </div>
          </div>
